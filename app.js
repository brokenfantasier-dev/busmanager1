...
function initializeCharts() {
    if (!document.getElementById("flowChart") || !document.getElementById("cumulativeChart")) return;
...
function resetState() {
    ...
    if (isAdmin() && flowChart && cumulativeChart) {
...
function renderUI() {
    ...
    const statusMap = {
        ...
        'stopped': { text: 'Tạm dừng', icon: 'fa-pause-circle', className: 'stopped' },
        'checking-out': { text: 'Đang kiểm tra', icon: 'fa-tasks', className: 'checking-out' },
    ...
    if (state.tripState === 'check-in' || state.tripState === 'checking-out') {
        elements.runningTimePanel.style.display = 'none';
        elements.countdownPanel.style.display = 'flex';
        const isCheckin = state.tripState === 'check-in';
        elements.countdownLabel.textContent = isCheckin ? 'Chờ ổn định' : 'Kiểm tra cuối chuyến';
        elements.countdownTimer.textContent = formatCountdown(isCheckin ? state.checkinCountdown : state.checkoutCountdown);
    } else {
        elements.runningTimePanel.style.display = 'flex';
...
function renderLogs() {
    elements.logEntries.innerHTML = state.logEntries.map(log => 
        `<div class="log-entry">
            <span class="log-timestamp">${log.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>
            <span class="log-type log-type-${log.type.toLowerCase()}">[${log.type}]</span>
...
function showAlert(title, message, isError) {
    ...
    elements.alertCloseButton.className = `dialog-action ${isError ? 'destructive' : 'primary'}`;
...
function initializeApp() {
    checkAuth();
    if(isAdmin()) {
        document.querySelectorAll('.admin-only').forEach(el => el.style.display = el.tagName.toLowerCase() === 'li' ? 'list-item' : 'grid');
        initializeCharts();
    } else {
    ...
    mainInterval = setInterval(async () => {
      if (!['in-progress', 'stopped'].includes(state.tripState)) return;
      
      // try {
      //   const response = await fetch(`${API_BASE}/status`);
      //   if (!response.ok) {
      //       addLog(`Lỗi kết nối backend: ${response.status}.`, 'WARNING');
      //   } else {
...
function mockDataGenerator() {
    ...
    const canGetOff = state.counts.getOn > state.counts.getOff;
    const newGetOff = canGetOff && Math.random() > 0.6 ? 1 : 0;

    if (newGetOn === 0 && newGetOff === 0) {
        return;
    }
